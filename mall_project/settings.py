...
import re
_raw_allowed = config('ALLOWED_HOSTS', default='https://honeycavemarket.com,www.honeycavemarket.com,honeycavemarket.com', cast=Csv())
ALLOWED_HOSTS = []
...
# ─── Email Configuration ───────────────────────────────────────────────────
#
#  IMPORTANT (Render free tier): outbound traffic to raw SMTP ports
#  (25/465/587) is blocked on Render's free web services. Any EMAIL_BACKEND
#  below that uses SMTP (Gmail, Outlook, Yahoo, custom) will fail with
#  "OSError: [Errno 101] Network is unreachable" until you either upgrade
#  to a paid Render plan, or switch to an HTTPS-based provider — SendGrid,
#  below, is the recommended default for that reason.
#
#  SendGrid (recommended — HTTPS API, works on Render's free tier):
#    pip install django-sendgrid-v5 (already in requirements.txt)
...
# SendGrid (django-sendgrid-v5) — only used when EMAIL_BACKEND is set to
# "sendgrid_backend.SendgridBackend". Harmless no-op otherwise.
SENDGRID_API_KEY                = config('SENDGRID_API_KEY', default='')
SENDGRID_SANDBOX_MODE_IN_DEBUG  = config('SENDGRID_SANDBOX_MODE_IN_DEBUG', default=False, cast=bool)
SENDGRID_ECHO_TO_STDOUT         = DEBUG   # mirror sent emails to logs while DEBUG=True
...
# ── Anthropic API (AI product recommendations + review summaries) ────────────
# Get a key at https://console.anthropic.com — used by mall/views.py for the
...
NALO_AUTH_KEY  = config('NALO_AUTH_KEY',  default='')  # Nalo's real issued API auth key, if using key-based auth instead of username/password

# ── Tiliow (secondary messaging: WhatsApp + SMS + OTP) ─────────────────────────
# Used only if Nalo isn't configured. Configure in Panel → Site Settings
# (recommended) or via these env vars. Site Settings take priority when tiliow_enabled.
...
# ── Logging ─────────────────────────────────────────────────────────────────
# Without this, Django swallows 500-error tracebacks when DEBUG=False —
# Render's logs would only show "500" with no detail. This prints full
# tracebacks to stdout/stderr, which Render captures and displays in Logs.
LOGGING = {
    'version': 1,
