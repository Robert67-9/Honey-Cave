...
Recommended providers for Ghana:
    - Nalo         (GHS billing, SMS via NaloSolutions API — primary provider)
...
How to configure Nalo:
    - NALO_USERNAME     (required)
    - NALO_PASSWORD     (required)
    - NALO_SENDER_ID    (optional, default: 'HoneyCave')
    - NALO_API_URL      (optional, default: https://sms.nalosolutions.com/smsbackend/clientapi/Resl_Nalo/send-message/)
...
def _load_nalo_config():
    """
    Nalo config for SMS. SiteSettings (admin-managed) first, env vars second.
    Returns (username, password, sender_id, api_url, auth_key) or
    (None, None, None, None, None).

    auth_key is Nalo's actual API auth key (Panel → Site Settings →
    "Nalo auth key"). When set, it's used in the "key" field of the
    request instead of the username — Nalo's API rejects requests with
    error 1713 "Invalid auth key" if a plain username is sent there
    instead of the real issued key.
    """
    try:
        from django.apps import apps
        SiteSettings = apps.get_model('mall', 'SiteSettings')
        s = SiteSettings.load()
        if getattr(s, 'nalo_enabled', False) and (getattr(s, 'nalo_username', '') or '').strip():
            return (
                (s.nalo_username or '').strip(),
                (s.nalo_password or '').strip(),
                (getattr(s, 'nalo_sender_id', '') or 'HoneyCave').strip(),
                (getattr(s, 'nalo_api_url', '') or 'https://sms.nalosolutions.com/smsbackend/clientapi/Resl_Nalo/send-message/').strip(),
                (getattr(s, 'nalo_auth_key', '') or '').strip(),
            )
    except Exception as e:
        logger.debug('SMS Nalo: SiteSettings unavailable, trying env: %s', e)
    username = (os.environ.get('NALO_USERNAME') or '').strip()
    password = (os.environ.get('NALO_PASSWORD') or '').strip()
    if not username or not password:
        return None, None, None, None, None
    sender_id = (os.environ.get('NALO_SENDER_ID') or 'HoneyCave').strip()
    api_url   = os.environ.get('NALO_API_URL', 'https://sms.nalosolutions.com/smsbackend/clientapi/Resl_Nalo/send-message/').strip()
    auth_key  = (os.environ.get('NALO_AUTH_KEY') or '').strip()
    return username, password, sender_id, api_url, auth_key
...
def send(phone, text):
    ...
    if not phone or not text:
        return False
    # Nalo is the primary provider — configured in Panel → Site Settings
    # (or via env vars). It carries SMS + OTP for every role.
    nalo_user, nalo_pass, _, _, _ = _load_nalo_config()
    if nalo_user and nalo_pass:
        return _send_nalo(phone, text)
    # Tiliow — secondary provider, still supported if configured.
...
def _send_nalo(phone, text):
    """
    Send SMS through the NaloSolutions REST API.
    Requires NALO_USERNAME and NALO_PASSWORD (Panel → Site Settings or env vars).
    Optionally NALO_AUTH_KEY — Nalo's real issued API key. When set, it's
    sent in the "key" field instead of the username, which is what Nalo's
    API actually expects (sending username there causes error 1713
    "Invalid auth key").

    POST https://sms.nalosolutions.com/smsbackend/clientapi/Resl_Nalo/send-message/
    Body (JSON): { "key": "<auth_key or username>", "msisdn": "<phone>",
                    "message": "<text>", "sender_id": "<sender>" }

    Nalo's exact response schema can vary slightly by account/gateway version;
    we treat any 2xx HTTP response without an explicit error code as success.
    """
    username, password, sender_id, api_url, auth_key = _load_nalo_config()
    if not username or not password:
        return False

    to_clean = _normalize_phone(phone)
    if not to_clean:
        logger.warning('SMS via Nalo: invalid phone %s', _mask_phone(phone))
        return False

    payload = {
        'username': username,
        'password': password,
        'key': auth_key or username,
        'msisdn': to_clean,
        'message': str(text)[:1600],
        'sender_id': sender_id or 'HoneyCave',
    }
    headers = {'Content-Type': 'application/json'}
    ok, info = _post_json(api_url, payload, headers)
    if ok:
        logger.info('SMS via Nalo sent to %s', _mask_phone(phone))
    else:
        logger.error('SMS via Nalo failed for %s: %s', _mask_phone(phone), info)
    return ok
