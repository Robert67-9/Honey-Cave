...
def admin_user_detail(request, pk):
    ...
    if request.method == 'POST':
        ...
        if action == 'toggle_staff':
            ...
            audit_log(request, 'user_fulfillment_officer', f'User "{target.username}"', f'is_fulfillment_officer={profile.is_fulfillment_officer}')
            messages.success(request, f'Fulfillment officer status updated for {target.username}.')
        return redirect('admin_user_detail', pk=pk)
    store_application = StoreApplication.objects.filter(applicant=target).order_by('-created').first()
    target_profile, _ = UserProfile.objects.get_or_create(user=target)
    return render(request, 'mall/admin/user_detail.html', {
        'target': target, 'orders': orders, 'store_application': store_application,
...
def admin_fulfillment_officer_add(request):
    ...
    if request.method == 'POST':
        try:
            ...
            # One-time login link — the admin can hand this to the officer
            # instead of (or alongside) the password so their first visit
            # doesn't require typing credentials. Burns on first use; every
            # visit after that goes through the normal login form.
            auto_login_token = OfficerAutoLoginToken.objects.create(
                officer=user, created_by=request.user,
            )
            auto_login_url = request.build_absolute_uri(
                f'/officer/auto-login/{auto_login_token.token}/'
            )
            ...
            messages.success(
                request,
                f'✓ Fulfillment Officer "{full_name}" registered. One-time login link (valid 72h, single use): '
                f'{auto_login_url} — or share the credentials — '
...
def admin_site_settings(request):
    ...
    if request.method == 'POST':
        try:
            ...
            # Optional ETA for a live countdown on the maintenance page.
            # Comes from a <input type="datetime-local"> field, so it's
            # naive local time -- make it timezone-aware before saving.
            # Blank clears the ETA (falls back to the plain text message).
            _eta_raw = _clean_str(request.POST, 'maintenance_eta', 40, required=False)
            if _eta_raw:
                from django.utils.dateparse import parse_datetime
                from django.utils import timezone as _tz
                _eta_dt = parse_datetime(_eta_raw)
                if _eta_dt is None:
                    raise ValueError('Maintenance ETA is not a valid date/time.')
                if _tz.is_naive(_eta_dt):
                    _eta_dt = _tz.make_aware(_eta_dt, _tz.get_current_timezone())
                settings_obj.maintenance_eta = _eta_dt
            else:
                settings_obj.maintenance_eta = None
            ...
            # Same blank-to-keep behaviour as the password field above.
            ...
            # Bust the maintenance middleware's shared cache immediately —
            # this must match the cache key used in models.py / middleware.py
            # so every gunicorn worker picks up the change right away.
            from django.core.cache import cache as _cache
            from .middleware import MaintenanceModeMiddleware
            _cache.delete(MaintenanceModeMiddleware._CACHE_KEY)
...
# ════════════════════════════════════════════════════════════════════════════
# Store Applications — public "Own a Store" applicants, admin approves/rejects.
# ════════════════════════════════════════════════════════════════════════════

@admin_required
def admin_store_applications(request):
    """List 'Own a Store' applications with status filters."""
    qs = StoreApplication.objects.select_related('applicant', 'decided_by').order_by('-created')
    ...
    counts = {
        'pending':  StoreApplication.objects.filter(status='pending').count(),
        'approved': StoreApplication.objects.filter(status='approved').count(),
        'rejected': StoreApplication.objects.filter(status='rejected').count(),
    }

    return render(request, 'mall/admin/store_applications.html', {
        'applications':  qs,
        'counts':        counts,
        'status_filter': status_filter,
    })
...
@admin_required
@require_POST
def admin_store_application_decide(request, pk):
    """Approve or reject a StoreApplication."""
    application = get_object_or_404(StoreApplication, pk=pk)
    decision = (request.POST.get('decision') or '').strip()
    ...
    if decision == 'approve':
        ...
        notif_title = '✅ Store application approved'
        notif_body = (
            f'Congratulations! Your application to open "{application.store_name}" has been approved. '
            f'You can now log in to the fulfillment officer portal.'
        )
        if note:
            notif_body += f'\n\nNote from admin: {note}'
    else:
        ...
        admin_note = note or f'Rejected by {request.user.username}'
        notif_title = '❌ Store application update'
        notif_body = f'Your application to open "{application.store_name}" was not approved this time.'
        if note:
            notif_body += f'\n\nReason: {note}'
    ...
    from .notify import notify
    sms_short = f'Honey Cave: Your store application was {decision}d.'
    if note:
        sms_short += f' Note: {note[:80]}'
    notify(
        ...
        title=notif_title,
        message=notif_body,
        link='/officer/login/' if decision == 'approve' else '/apply/store/',
        whatsapp_text=notif_body,
        sms_text=sms_short,
    )
    ...
    audit_log(
        request,
        f'store_application_{decision}',
        f'{application.applicant.username} — {application.store_name}',
    )
...
# ─── Wallets & Payouts (dispute resolution) ────────────────────────────────
# Gives admin visibility into every seller/rider wallet — balances, full
# ledger history, and withdrawal status — so a "why haven't I been paid" /
# "this amount looks wrong" dispute can be resolved without touching the
# database directly. Every mutating action here goes through mall/wallet.py
# (never edits Wallet balance fields by hand) and is audit-logged.

@admin_required
def admin_wallets(request):
    """List all seller + rider wallets with balances, filterable by type/status."""
    owner_type = request.GET.get('type') or ''
    flag       = request.GET.get('flag') or ''
    q          = (request.GET.get('q') or '').strip()

    qs = Wallet.objects.select_related('seller', 'rider').order_by('-updated_at')
    if owner_type in ('seller', 'rider'):
        qs = qs.filter(owner_type=owner_type)
    if flag == 'frozen':
        qs = qs.filter(is_frozen=True)
    elif flag == 'untrusted':
        qs = qs.filter(is_trusted=False)
    if q:
        qs = qs.filter(
            Q(seller__username__icontains=q) | Q(seller__first_name__icontains=q) |
            Q(seller__last_name__icontains=q) | Q(rider__name__icontains=q) | Q(rider__phone__icontains=q)
        )

    counts = {
        'all':       Wallet.objects.count(),
        'sellers':   Wallet.objects.filter(owner_type='seller').count(),
        'riders':    Wallet.objects.filter(owner_type='rider').count(),
    ...
    return render(request, 'mall/admin/wallets.html', {
        'page_obj':   page_obj,
        'counts':     counts,
...
@admin_required
def admin_wallet_detail(request, pk):
    """
    Full transaction history + withdrawal history for one wallet, plus
    admin actions: freeze/unfreeze, mark trusted/untrusted, waive fee,
    manual balance adjustment, and force-reverse a stuck withdrawal.
    This is the "resolve a dispute" screen.
    """
    wallet = get_object_or_404(Wallet.objects.select_related('seller', 'rider'), pk=pk)
    ...
    if request.method == 'POST':
        ...
        if action == 'toggle_frozen':
            ...
            audit_log(request, 'wallet_freeze_toggle', wallet, f'is_frozen={wallet.is_frozen}')
            messages.success(request, f'Wallet {"frozen" if wallet.is_frozen else "unfrozen"}.')

        elif action == 'toggle_trusted':
            wallet.is_trusted = not wallet.is_trusted
            wallet.save(update_fields=['is_trusted'])
            audit_log(request, 'wallet_trusted_toggle', wallet, f'is_trusted={wallet.is_trusted}')
            messages.success(request, f'Wallet marked {"trusted" if wallet.is_trusted else "untrusted"}.')

        elif action == 'toggle_fee_waived':
            wallet.fee_waived = not wallet.fee_waived
            wallet.save(update_fields=['fee_waived'])
            audit_log(request, 'wallet_fee_waived_toggle', wallet, f'fee_waived={wallet.fee_waived}')
            messages.success(request, 'Withdrawal fee waiver updated.')

        elif action == 'adjustment':
            try:
                amount = Decimal(request.POST.get('amount', '0') or '0')
                note = _clean_str(request.POST, 'note', max_len=300)
            except (InvalidOperation, ValueError) as e:
                messages.error(request, str(e))
                return redirect('admin_wallet_detail', pk=pk)
            if amount == 0:
                messages.error(request, 'Adjustment amount cannot be zero.')
                return redirect('admin_wallet_detail', pk=pk)
            WalletTransaction.objects.create(
                wallet=wallet, type='adjustment', status='available',
                amount=amount, note=note, created_by=request.user,
            )
            Wallet.objects.filter(pk=wallet.pk).update(available_balance=F('available_balance') + amount)
            audit_log(request, 'wallet_manual_adjustment', wallet, f'{amount:+} — {note}')
            messages.success(request, f'Manual adjustment of GH₵{amount:+} recorded.')

        elif action == 'reverse_withdrawal':
            wr = get_object_or_404(WithdrawalRequest, pk=request.POST.get('withdrawal_id'), wallet=wallet)
            reason = _clean_str(request.POST, 'reason', max_len=300, required=False) or 'Manually reversed by admin'
            try:
                wallet_svc.reverse_withdrawal(wr, reason=reason)
                audit_log(request, 'withdrawal_force_reversed', wr, reason)
                messages.success(request, f'Withdrawal {wr.unique_reference} reversed — funds returned to available balance.')
            except wallet_svc.WalletError as e:
                messages.error(request, str(e))

        return redirect('admin_wallet_detail', pk=pk)

    transactions = wallet.transactions.select_related('order', 'created_by')[:100]
    withdrawals  = wallet.withdrawals.all()[:30]

    return render(request, 'mall/admin/wallet_detail.html', {
        'wallet':       wallet,
...
def admin_withdrawals(request):
    """Cross-wallet queue of withdrawal requests — the fastest way to spot stuck payouts."""
    status = request.GET.get('status') or ''
    qs = WithdrawalRequest.objects.select_related('wallet__seller', 'wallet__rider').order_by('-requested_at')
    if status:
        qs = qs.filter(status=status)

    counts = {
        'all':          WithdrawalRequest.objects.count(),
        'pending_otp':  WithdrawalRequest.objects.filter(status='pending_otp').count(),
        ...
        'failed':       WithdrawalRequest.objects.filter(status='failed').count(),
    }

    paginator = Paginator(qs, 40)
    page_obj = paginator.get_page(request.GET.get('page'))

    return render(request, 'mall/admin/withdrawals.html', {
        'page_obj': page_obj,
        'counts':   counts,
        'status':   status,
    })
