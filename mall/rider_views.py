...
def rider_withdraw(request):
    """Step 1 — rider requests a cash-out, we send an OTP before touching Paystack."""
    wallet = wallet_svc.get_or_create_rider_wallet(request.rider)
    wallet_svc.release_matured_pending(wallet)
    wallet.refresh_from_db()
    site = SiteSettings.load()
...
def rider_withdraw_confirm(request):
    ...
    otp_id = request.session.get('rider_withdrawal_otp_id')
    if not wr_id or not otp_id:
        return redirect('rider_withdraw')

    wr = WithdrawalRequest.objects.filter(id=wr_id, wallet__rider=request.rider).first()
    otp = RiderOTP.objects.filter(id=otp_id, rider=request.rider, purpose='withdrawal').first()
