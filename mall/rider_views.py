...
# Riders earn a share of the delivery fee per completed home-delivery
# order (see mall/handoff.py:advance_after_verify → mall/wallet.py). Cash
# out via Paystack (bank or mobile money), gated by an OTP sent the same
# way rider login codes are (RiderOTP, purpose='withdrawal').

def _rider_period_totals(wallet):
    now = timezone.now()
...
def rider_earnings(request):
    """Rider earnings dashboard — mirrors the seller one in fulfillment_officer_views.py."""
    wallet = wallet_svc.get_or_create_rider_wallet(request.rider)
    wallet_svc.release_matured_pending(wallet)
    wallet.refresh_from_db()

    totals = _rider_period_totals(wallet)
    ...
    return render(request, 'mall/rider/earnings.html', {
        'wallet': wallet,
        'totals': totals,
        'recent_tx': recent_tx,
...
def _send_rider_withdrawal_otp(request, wr):
    """
    ...
    caller uses this to warn the rider instead of leaving them stranded on
    the confirm page waiting for a code that never arrives.
    """
    code = f'{secrets.randbelow(1_000_000):06d}'
    RiderOTP.objects.filter(
        rider=request.rider, purpose='withdrawal', used_at__isnull=True, locked=False,
    ).update(locked=True)
    otp = RiderOTP.objects.create(
        rider=request.rider, purpose='withdrawal',
        code_hash=RiderOTP.hash_code(code),
        expires_at=timezone.now() + timezone.timedelta(minutes=RiderOTP.EXPIRY_MINUTES),
        requested_ip=_client_ip(request),
    )
    sent = False
    try:
        result = _send_otp(request.rider, code)
        sent = bool(result and any(result.values()))
    except Exception as e:
        logger.warning('Rider withdrawal OTP send failed for %s: %s', request.rider.phone, e)
    request.session['rider_withdrawal_id'] = wr.id
    request.session['rider_withdrawal_otp_id'] = otp.id
    return sent
...
def rider_withdraw(request):
    """
    Step 1 — rider requests a cash-out, we send an OTP before touching
    Paystack. If a still-live pending_otp request already exists for this
    wallet (they clicked "Cash Out" again after losing their session or
    not getting the code), resume that one instead of creating another
    stuck duplicate.
    """
    wallet = wallet_svc.get_or_create_rider_wallet(request.rider)
    wallet_svc.release_matured_pending(wallet)
    ...
    if resumable and request.method == 'GET':
        request.session['rider_withdrawal_id'] = resumable.id
        messages.info(request, 'You already have a cash-out awaiting confirmation.')
        return redirect('rider_withdraw_confirm')

    if request.method == 'POST':
        if not check_rate_limit('rider_withdraw_request', request, limit=5, window=600):
        ...
        try:
            ...
            return redirect('rider_withdraw')
        ...
        sent = _send_rider_withdrawal_otp(request, wr)
        if not sent:
            messages.warning(
                request,
    ...
    return render(request, 'mall/rider/withdraw.html', {
        'wallet': wallet, 'site': site, 'banks': GHANA_BANKS,
    })


@rider_required
def rider_withdraw_resume(request, pk):
    """
    Link from the earnings page's withdrawal table — lets a rider get back
    to the OTP entry screen for one of their own still-live pending_otp
    requests after losing their session.
    """
    wr = get_object_or_404(WithdrawalRequest, pk=pk, wallet__rider=request.rider, status='pending_otp')
    request.session['rider_withdrawal_id'] = wr.id
    request.session.pop('rider_withdrawal_otp_id', None)
    return redirect('rider_withdraw_confirm')


@rider_required
@require_POST
def rider_withdraw_resend(request):
    """Resend a fresh OTP for the withdrawal currently in this session."""
    wr_id = request.session.get('rider_withdrawal_id')
    wr = WithdrawalRequest.objects.filter(id=wr_id, wallet__rider=request.rider, status='pending_otp').first()
    if wr is None:
        messages.error(request, 'This withdrawal request is no longer valid. Please start again.')
        return redirect('rider_withdraw')

    if not check_rate_limit('rider_withdraw_resend', request, limit=5, window=600):
    ...
    if sent:
        ...
        messages.warning(
            request,
            "We couldn't confirm delivery. Double-check your registered phone number, "
...
@rider_required
@require_http_methods(['GET', 'POST'])
def rider_withdraw_confirm(request):
    """Step 2 — rider enters the OTP to authorise the payout."""
    wr_id = request.session.get('rider_withdrawal_id')
    if not wr_id:
        return redirect('rider_withdraw')

    wr = WithdrawalRequest.objects.filter(id=wr_id, wallet__rider=request.rider).first()
    # Most recent unused, unlocked OTP for this rider/purpose — so a resend
    # always "just works" even if the session's otp id is stale.
    otp = (RiderOTP.objects
           .filter(rider=request.rider, purpose='withdrawal', used_at__isnull=True, locked=False)
           .order_by('-created_at').first())
    ...
    if request.method == 'POST':
        if not check_rate_limit('rider_withdraw_verify', request, limit=8, window=600):
            messages.error(request, 'Too many attempts. Please wait a few minutes.')
            return render(request, 'mall/rider/withdraw_confirm.html', {'wr': wr})

        entered = (request.POST.get('code') or '').strip()
        if otp.used_at is not None or otp.locked or otp.is_expired:
            messages.error(request, 'This code is no longer valid. Please request a new one.')
            return redirect('rider_withdraw')

        if not constant_time_compare(RiderOTP.hash_code(entered), otp.code_hash):
            otp.attempts += 1
            if otp.attempts >= RiderOTP.MAX_ATTEMPTS:
                otp.locked = True
                otp.save(update_fields=['attempts', 'locked'])
                messages.error(request, 'Too many wrong codes. Please request a new one.')
                return redirect('rider_withdraw')
            otp.save(update_fields=['attempts'])
            messages.error(request, f'Wrong code. {otp.remaining_attempts} attempt(s) left.')
            return render(request, 'mall/rider/withdraw_confirm.html', {'wr': wr})
        ...
        request.session.pop('rider_withdrawal_id', None)
        request.session.pop('rider_withdrawal_otp_id', None)
