...
def google_login_start(request):
    ...
    Requires a POST with agree_terms checked — the same Terms & Conditions /
    Privacy Policy agreement required on the email/password login and signup
    forms. This is enforced server-side (not just via the button's JS check)
    so the OAuth shortcut can't be used to skip the agreement.
    """
    ...
    if request.method != 'POST' or not request.POST.get('agree_terms'):
        messages.error(request, 'You must agree to the Terms & Conditions and Privacy Policy to continue.')
        return redirect('login')
