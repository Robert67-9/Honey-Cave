...
from .models import ShipmentBooking, ShipmentTrackingEvent, StoreApplication


class ShipmentTrackingEventInline(admin.TabularInline):
    model = ShipmentTrackingEvent
    extra = 1
    fields = ['order', 'label', 'location', 'description', 'status', 'completed']


@admin.register(ShipmentBooking)
class ShipmentBookingAdmin(admin.ModelAdmin):
    list_display  = ['tracking_code', 'shipping_method', 'sender_name', 'recipient_name', 'dest_city', 'status', 'total_cost', 'created']
    list_filter   = ['shipping_method', 'status', 'delivery_option', 'package_type']
    list_editable = ['status']
    search_fields = ['tracking_code', 'sender_name', 'recipient_name', 'sender_phone', 'recipient_phone']
    readonly_fields = ['tracking_code', 'base_price', 'weight_fee', 'speed_multiplier', 'insurance_fee', 'total_cost', 'created', 'updated']
