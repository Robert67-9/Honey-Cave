...
def officer_withdraw(request):
    ...
    if request.method == 'POST':
        ...
        # Send OTP to confirm — reuses the site's existing OTP delivery
        # (email via account, SMS via profile phone if set).
        otp, plaintext = OTPVerification.create_for(request.user, purpose='withdrawal')
        try:
            from . import sms as _sms
            profile = getattr(request.user, 'profile', None)
            if profile and profile.phone:
                _sms.send(profile.phone, f'Honey Cave Market — your cash-out confirmation code is {plaintext}. Valid 10 minutes.')
...
def officer_withdraw_confirm(request):
    ...
    otp_id = request.session.get('officer_withdrawal_otp_id')
    if not wr_id or not otp_id:
        return redirect('officer_withdraw')

    wr = WithdrawalRequest.objects.filter(id=wr_id, wallet__seller=request.user).first()
    otp = OTPVerification.objects.filter(id=otp_id, user=request.user, purpose='withdrawal').first()
