...
from .security import validate_uploaded_image, check_rate_limit
...
def officer_auto_login(request, token):
    """
    One-time entry point for a freshly-assigned officer: admin shares this
    link instead of (or alongside) the generated password. First visit logs
    the officer straight in and burns the token; every visit after that goes
    through the normal /officer/login/ form.
    """
    from .models import OfficerAutoLoginToken

    row = OfficerAutoLoginToken.objects.filter(token=token).select_related('officer', 'officer__profile').first()
    if row is None or not row.is_valid:
        messages.error(request, 'This login link is invalid or has expired. Please log in with your username and password.')
        return redirect('/officer/login/')

    prof = getattr(row.officer, 'profile', None)
    if not prof or not prof.is_fulfillment_officer:
        messages.error(request, 'This account is not authorised for the fulfillment officer portal.')
        return redirect('/officer/login/')
    ...
    auth_login(request, row.officer)
    messages.success(request, f'Welcome, {row.officer.first_name or row.officer.username}! You\'re logged in — set a password you\'ll remember under your profile whenever you like.')
...
# "Seller" here is a fulfillment officer / store owner who owns products via
# Product.created_by. Revenue is earned per delivered order (see
# mall/handoff.py:advance_after_verify → mall/wallet.py:credit_order_earnings)
# and cashed out here via Paystack (bank or mobile money), gated by OTP.

def _seller_period_totals(wallet):
    """Revenue credited today / this week / this month / this year."""
    now = timezone.now()
    ...
    return {
        'today': wallet.earnings_between(today_start, now),
        'week':  wallet.earnings_between(week_start, now),
        'month': wallet.earnings_between(month_start, now),
        'year':  wallet.earnings_between(year_start, now),
    }


@fulfillment_officer_required
def officer_earnings(request):
    """
    Seller earnings dashboard: revenue today/week/month/year, current
    pending/available/withdrawn/reserve balances, and recent ledger
    activity. Also releases any matured holds so balances shown are current.
    """
    wallet = wallet_svc.get_or_create_seller_wallet(request.user)
    wallet_svc.release_matured_pending(wallet)
    wallet.refresh_from_db()

    totals = _seller_period_totals(wallet)
    recent_tx = wallet.transactions.select_related('order')[:30]
    recent_withdrawals = wallet.withdrawals.all()[:10]
    site = SiteSettings.load()

    from .payments.transfers import GHANA_BANKS
    return render(request, 'mall/fulfillment_officer/earnings.html', {
        'wallet': wallet,
        'totals': totals,
        'recent_tx': recent_tx,
...
def _send_officer_withdrawal_otp(request, wr):
    """
    Create + send a fresh OTP for this withdrawal request. Returns True if
    at least one delivery channel reported success, False otherwise — the
    caller uses this to warn the seller instead of leaving them stranded
    on the confirm page waiting for a code that never arrives.
    """
    otp, plaintext = OTPVerification.create_for(request.user, purpose='withdrawal')
    sent = False
    try:
        from . import sms as _sms
        profile = getattr(request.user, 'profile', None)
        if profile and profile.phone:
            sent = _sms.send(profile.phone, f'Honey Cave Market — your cash-out confirmation code is {plaintext}. Valid 10 minutes.') or sent
        _sms.console_otp(profile.phone if profile else request.user.email, plaintext, label='Withdrawal confirmation code')
    except Exception as e:
        logger.warning('Withdrawal OTP send failed for %s: %s', request.user.username, e)
    request.session['officer_withdrawal_id'] = wr.id
    request.session['officer_withdrawal_otp_id'] = otp.id
    return sent


@fulfillment_officer_required
@require_http_methods(['GET', 'POST'])
def officer_withdraw(request):
    """
    Step 1 — seller fills in amount + payout destination. On submit,
    creates a WithdrawalRequest in 'pending_otp' state and sends an OTP
    (via OTPVerification, purpose='withdrawal') to the seller's account
    email/phone before anything is sent to Paystack.

    If a still-live pending_otp request already exists for this wallet
    (they clicked "Cash Out" again after losing their session or not
    getting the code), resume that one instead of creating another stuck
    duplicate.
    """
    wallet = wallet_svc.get_or_create_seller_wallet(request.user)
    wallet_svc.release_matured_pending(wallet)
    ...
    resumable = wallet_svc.find_resumable_withdrawal(wallet)
    if resumable and request.method == 'GET':
        request.session['officer_withdrawal_id'] = resumable.id
        messages.info(request, 'You already have a cash-out awaiting confirmation.')
        return redirect('officer_withdraw_confirm')

    if request.method == 'POST':
        if not check_rate_limit('officer_withdraw_request', request, limit=5, window=600):
            messages.error(request, 'Too many withdrawal attempts. Please wait a few minutes.')
            return redirect('officer_earnings')

        if resumable:
            request.session['officer_withdrawal_id'] = resumable.id
            messages.info(request, 'You already have a cash-out awaiting confirmation — resuming that one.')
            return redirect('officer_withdraw_confirm')
        ...
        try:
            ...
            return redirect('officer_withdraw')

        destination = {}
        if method == 'bank':
            destination = {
                'bank_name': request.POST.get('bank_code', ''),
            ...
            destination = {
                ...
                'account_number': request.POST.get('momo_number', '').strip(),
                'account_name': request.POST.get('account_name', '').strip(),
            }

        try:
            wr = wallet_svc.request_withdrawal(
                wallet, amount=amount, method=method, destination=destination,
            )
        except wallet_svc.WalletError as e:
            messages.error(request, str(e))
            return redirect('officer_withdraw')

        sent = _send_officer_withdrawal_otp(request, wr)
        if not sent:
            messages.warning(
                request,
                "We couldn't confirm the code was delivered to your phone. "
                "If it doesn't arrive shortly, use \"Resend code\" on the next page."
            )
        return redirect('officer_withdraw_confirm')

    return render(request, 'mall/fulfillment_officer/withdraw.html', {
        'wallet': wallet, 'site': site, 'banks': GHANA_BANKS,
    })


@fulfillment_officer_required
def officer_withdraw_resume(request, pk):
    """
    Link from the earnings page's withdrawal table — lets a seller get
    back to the OTP entry screen for one of their own still-live
    pending_otp requests after losing their session.
    """
    wr = get_object_or_404(WithdrawalRequest, pk=pk, wallet__seller=request.user, status='pending_otp')
    request.session['officer_withdrawal_id'] = wr.id
    request.session.pop('officer_withdrawal_otp_id', None)  # force a fresh OTP lookup on the confirm page
    return redirect('officer_withdraw_confirm')


@fulfillment_officer_required
@require_POST
def officer_withdraw_resend(request):
    """Resend a fresh OTP for the withdrawal currently in this session."""
    wr_id = request.session.get('officer_withdrawal_id')
    wr = WithdrawalRequest.objects.filter(id=wr_id, wallet__seller=request.user, status='pending_otp').first()
    if wr is None:
        messages.error(request, 'This withdrawal request is no longer valid. Please start again.')
        return redirect('officer_withdraw')

    if not check_rate_limit('officer_withdraw_resend', request, limit=5, window=600):
        messages.error(request, 'Too many resend attempts. Please wait a few minutes.')
        return redirect('officer_withdraw_confirm')
    ...
    if sent:
        ...
        messages.warning(
            request,
            "We couldn't confirm delivery. Double-check your registered phone number in your profile, "
...
@fulfillment_officer_required
@require_http_methods(['GET', 'POST'])
def officer_withdraw_confirm(request):
    """Step 2 — seller enters the OTP to actually authorise the payout."""
    wr_id = request.session.get('officer_withdrawal_id')
    if not wr_id:
        return redirect('officer_withdraw')

    wr = WithdrawalRequest.objects.filter(id=wr_id, wallet__seller=request.user).first()
    # Look up the most recent unused OTP for this user/purpose rather than
    # a session-stored id — this way a resend always "just works" even if
    # the session's otp id is stale.
    otp = (OTPVerification.objects
           .filter(user=request.user, purpose='withdrawal', is_used=False)
           .order_by('-created_at').first())

    if wr is None or otp is None or wr.status != 'pending_otp':
        messages.error(request, 'This withdrawal request is no longer valid. Please start again.')
        return redirect('officer_withdraw')

    if request.method == 'POST':
        if not check_rate_limit('officer_withdraw_verify', request, limit=8, window=600):
            messages.error(request, 'Too many attempts. Please wait a few minutes.')
            return render(request, 'mall/fulfillment_officer/withdraw_confirm.html', {'wr': wr})

        entered = (request.POST.get('code') or '').strip()
        if not otp.is_valid() or not otp.matches(entered):
            messages.error(request, 'Incorrect or expired code. Please try again, or use "Resend code" below.')
            return render(request, 'mall/fulfillment_officer/withdraw_confirm.html', {'wr': wr})

        otp.is_used = True
        otp.save(update_fields=['is_used'])
        try:
            wallet_svc.confirm_withdrawal_otp(wr)
        except wallet_svc.WalletError as e:
            messages.error(request, str(e))
            return redirect('officer_earnings')

        request.session.pop('officer_withdrawal_id', None)
        request.session.pop('officer_withdrawal_otp_id', None)
        messages.success(request, f'Withdrawal of GH₵{wr.amount} submitted — you\'ll be notified once it lands.')
        return redirect('officer_earnings')

    return render(request, 'mall/fulfillment_officer/withdraw_confirm.html', {'wr': wr})


# ─── Product Boosts (seller-paid promotion) ────────────────────────────────

@fulfillment_officer_required
def officer_boosts(request):
    """
    Seller picks one of their products and a tier (Boost/Spotlight/Featured)
    to promote it in listings for a limited period. Payment is either
    straight from the seller's wallet (available_balance) or by card via
    Paystack — seller's choice.
    """
    products = Product.objects.filter(created_by=request.user).order_by('-created')
    active_boosts = (ProductBoost.objects
                      .filter(seller=request.user, status='active')
                      .select_related('product'))
    past_boosts = (ProductBoost.objects
    ...
    pending_boost = None
    pending_boost_id = request.session.get('pending_boost_id')
    if pending_boost_id:
        pending_boost = (ProductBoost.objects
                          .filter(pk=pending_boost_id, seller=request.user, status='pending_payment')
                          .select_related('product').first())
        if pending_boost is None:
            request.session.pop('pending_boost_id', None)

    return render(request, 'mall/fulfillment_officer/boosts.html', {
        'products': products,
        'active_boosts': active_boosts,
        'past_boosts': past_boosts,
        'wallet': wallet,
        'tiers': ProductBoost.TIER_CHOICES,
        'tier_days': ProductBoost.TIER_DAYS,
        'tier_prices': ProductBoost.TIER_PRICE,
        'paystack_public_key': paystack_public_key,
        'paystack_configured': paystack_configured,
        'officer_email': request.user.email or '',
        'pending_boost': pending_boost,
        'pending_boost_amount_pesewas': int(pending_boost.amount * 100) if pending_boost else 0,
    })


@fulfillment_officer_required
@require_POST
def officer_boost_start(request):
    """
    Seller submits product + tier + payment method. If paying from wallet,
    debit and activate immediately. If paying by card, create the
    pending_payment ProductBoost and hand back its id — the page then opens
    the Paystack popup and calls officer_boost_verify on success.
    """
    product = get_object_or_404(Product, pk=request.POST.get('product_id'), created_by=request.user)
    tier = request.POST.get('tier', '')
    method = request.POST.get('method', 'card')

    if tier not in ProductBoost.TIER_DAYS:
        messages.error(request, 'Choose a valid boost tier.')
    ...
    boost = ProductBoost.objects.create(
        seller=request.user, product=product, tier=tier, amount=amount,
    ...
    if method == 'wallet':
        wallet = wallet_svc.get_or_create_seller_wallet(request.user)
        try:
            wallet_svc.purchase_boost_from_wallet(wallet, boost)
        except wallet_svc.WalletError as e:
            boost.status = 'cancelled'
            boost.save(update_fields=['status'])
            messages.error(request, str(e))
            return redirect('officer_boosts')
        messages.success(request, f'"{product.name}" is now boosted ({boost.get_tier_display()}).')
        return redirect('officer_boosts')

    # Card payment — front-end opens the Paystack popup for this boost.id
    request.session['pending_boost_id'] = boost.id
    return redirect('officer_boosts')


@fulfillment_officer_required
@require_POST
def officer_boost_verify(request):
    """AJAX: verify a Paystack card payment for a pending ProductBoost."""
    from .payments import dispatch as _dispatch
    import json as _json
    ...
    try:
        body = _json.loads(request.body)
        reference = (body.get('reference') or '').strip()
        boost_id = body.get('boost_id')
    except Exception:
        return JsonResponse({'verified': False, 'error': 'Invalid request.'}, status=400)

    boost = ProductBoost.objects.filter(pk=boost_id, seller=request.user, status='pending_payment').first()
    if boost is None:
        return JsonResponse({'verified': False, 'error': 'No pending boost payment found.'}, status=400)
    ...
    required_pesewas = int(boost.amount * 100)
    if result.amount_pesewas < required_pesewas:
        return JsonResponse({
            'verified': False,
            'error': f'Amount paid (GH₵{result.amount_pesewas/100:.2f}) is less than the required GH₵{boost.amount:.2f}.',
        }, status=400)

    boost.payment_reference = reference
    boost.save(update_fields=['payment_reference'])
    wallet_svc.activate_boost(boost)
