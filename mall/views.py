...
def send_login_alert(user, *, ip, user_agent):
    """
    Email the user right after a successful login with the time, IP address,
    and device/browser info, so they can flag anything they don't recognize.

    Best-effort only — never raises. A failed alert email should never block
    or roll back a successful login.
    """
    from django.utils import timezone

    when = timezone.localtime().strftime('%d %b %Y, %I:%M %p')
    subject = 'Honey Cave Market — New Sign-In to Your Account'
    plain_msg = (
        f'Hi {user.first_name or user.username},\n\n'
        f'Your account was just signed in to:\n\n'
        f'  Time:   {when}\n'
        f'  IP:     {ip or "unknown"}\n'
        f'  Device: {user_agent or "unknown"}\n\n'
        f"If this was you, no action is needed.\n"
        f'If you do not recognize this activity, please reset your password '
        f'immediately and contact us.\n\n'
        f'— Honey Cave Market'
    )
    send_mail(
        subject, plain_msg,
...
def product_list(request):
    ...
    boosted_ids      = set(ProductBoost.boosted_product_ids())

    def _location_map(product_list_):
        """product_id -> short 'Ships from' string, built from the prefetched branches."""
        out = {}
        for p in product_list_:
            branches = getattr(p, 'live_branch_products', [])
            if not branches:
                continue
            names = [bp.branch.name for bp in branches]
            out[p.id] = names[0] if len(names) == 1 else f'{names[0]} +{len(names) - 1} more'
        return out
    ...
    if sort in ('price_asc', 'price_desc', 'newest', 'name'):
        ...
        return render(request, 'mall/product_list.html', {
            ...
            'branch_map': _location_map(page_obj),
        })
    else:
        ...
        # Boosted products (paid promotion) rank first, ahead of the normal
        # stock-based shuffle — that's the whole point of paying for a boost.
        boosted_items    = [p for p in items if p.id in boosted_ids]
        unboosted_items  = [p for p in items if p.id not in boosted_ids]
        rng.shuffle(boosted_items)
        # Split into tiers so well-stocked items stay near top but shuffle within tier
        in_stock  = [p for p in unboosted_items if p.stock > 0]
        out_stock = [p for p in unboosted_items if p.stock == 0]
        rng.shuffle(in_stock)
        rng.shuffle(out_stock)
        from django.core.paginator import Paginator as _Pag
        all_items = boosted_items + in_stock + out_stock
...
def product_detail(request, slug):
    ...
    # Where this product ships from — the branch(es) that actually stock it,
    # so the customer knows where it's coming from before they buy.
    stocking_branches = (
        product.branch_pricing
        .filter(is_available=True, branch__is_active=True)
        .select_related('branch')
        .order_by('branch__region', 'branch__name')
    )
    ...
    return render(request, 'mall/product_detail.html', {
        ...
        'stocking_branches':   stocking_branches,
        'is_boosted':          product.boosts.filter(status='active').exists(),
...
def _handle_provider_webhook(request, provider_slug):
    ...
    if not result.is_payment_event and not result.is_transfer_event:
        return JsonResponse({'status': result.response_body or 'ok'}, status=result.http_status)

    # Payout event — mark the matching WithdrawalRequest completed/failed.
    # This is the ONLY place a withdrawal is marked 'completed' — never
    # trust the synchronous transfer API response for that.
    if result.is_transfer_event:
        from .models import WithdrawalRequest
        from . import wallet as _wallet
        wr = WithdrawalRequest.objects.filter(unique_reference=result.reference).first()
        if wr is None:
            return JsonResponse({'status': 'ok — unknown withdrawal reference'}, status=200)
        try:
            if result.event_type == 'transfer.success':
                _wallet.mark_withdrawal_completed(wr)
            elif result.event_type in ('transfer.failed', 'transfer.reversed'):
                _wallet.reverse_withdrawal(wr, reason=f'Paystack {result.event_type}')
        except Exception as e:
            logger.error('Withdrawal webhook processing failed for %s: %s', result.reference, e)
...
def login_view(request):
    ...
    if request.method == 'POST' and form.is_valid():
        ...
        # Alert the user of this login (time/IP/device) so they can flag
        # anything unrecognized. Best-effort — never blocks the login.
        try:
            ip = (request.META.get('HTTP_X_FORWARDED_FOR', '').split(',')[0].strip()
                  or request.META.get('REMOTE_ADDR'))
            send_login_alert(user, ip=ip, user_agent=request.META.get('HTTP_USER_AGENT', ''))
        except Exception as e:
            logger.warning('Login alert email failed for user %s: %s', user.pk, e)
...
def apply_job(request):
    return render(request, 'mall/apply_job.html')


# ─── Shipping (HCL) Import / Export ───────────────────────────────────────────

# Feature flag: the HCL Import/Export shipping product isn't launched yet.
# Flip this to False when it's ready to go live — no other code changes needed,
# the booking/track endpoints below re-check it too so they can't be hit directly.
SHIPPING_COMING_SOON = True


def shipping_import_export(request):
    if SHIPPING_COMING_SOON:
        return render(request, 'mall/shipping_coming_soon.html')
    return render(request, 'mall/shipping_import_export.html', {
        'delivery_options': SHIPPING_DELIVERY_OPTIONS,
        'shipping_methods': SHIPPING_METHODS,
    })


@require_POST
def shipping_book(request):
    """AJAX: validate + price a shipment booking server-side, create it, return the tracking code."""
    if SHIPPING_COMING_SOON:
        return JsonResponse({'ok': False, 'errors': {'__all__': ['Shipping is coming soon — bookings aren\'t open yet.']}}, status=403)

    form = ShipmentBookingForm(request.POST)
    if not form.is_valid():
        return JsonResponse({'ok': False, 'errors': form.errors}, status=400)

    if not check_rate_limit('shipping_book', request, limit=5, window=3600):
        return JsonResponse({'ok': False, 'errors': {'__all__': ['Too many bookings — please try again later.']}}, status=429)

    booking = form.save(commit=False)
    estimate = calculate_shipment_estimate(
        booking.weight_kg, booking.delivery_option, booking.package_type, booking.insurance_required,
        shipping_method=booking.shipping_method,
    )
    # calculate_shipment_estimate corrects a mismatched method/option pair — keep the booking consistent with what was actually priced.
    booking.shipping_method = estimate['shipping_method']
    booking.delivery_option = estimate['delivery_option']
    booking.base_price = estimate['base_price']
    booking.weight_fee = estimate['weight_fee']
    booking.speed_multiplier = estimate['speed_multiplier']
    booking.insurance_fee = estimate['insurance_fee']
    booking.total_cost = estimate['total_cost']

    # Generate a unique HC-XXXXXXX tracking code.
    for _ in range(10):
        code = f'HC-{random.randint(1000000, 9999999)}'
        if not ShipmentBooking.objects.filter(tracking_code=code).exists():
            booking.tracking_code = code
            break
    else:
        return JsonResponse({'ok': False, 'errors': {'__all__': ['Could not generate a tracking code — please try again.']}}, status=500)

    booking.current_location = f'{booking.pickup_city}, {booking.pickup_country} — Origin Facility'
    if request.user.is_authenticated:
    ...
    ShipmentTrackingEvent.objects.create(
        booking=booking,
        label='Just now',
        location=f'{booking.pickup_city}, {booking.pickup_country}',
        description='Shipment booked — awaiting pickup confirmation',
        status='picked_up',
        completed=True,
        order=0,
    )

    from .notify import notify_admins, notify_phone, notify
    notify_admins(
        notif_type='order_update',
        title='📦 New Shipping (HCL) booking',
        message=f'{booking.sender_name} booked shipment {booking.tracking_code} to {booking.dest_city}, {booking.dest_country}.',
        link='/panel/',
    )

    # ── Confirm to the customer: email + WhatsApp/SMS with the tracking code ──
    track_url = request.build_absolute_uri('/shipping-import-export/')
    total_str = f'{booking.total_cost:.2f}'
    email_body = (
        f'Hi {booking.sender_name.split()[0] if booking.sender_name else "there"},\n\n'
        f'Your shipment has been booked with Honey Cave Shipping (HCL).\n\n'
        f'Tracking Number: {booking.tracking_code}\n'
        f'Route: {booking.pickup_city}, {booking.pickup_country} → {booking.dest_city}, {booking.dest_country}\n'
        f'Method: {booking.get_shipping_method_display()} — {booking.get_delivery_option_display()}\n'
        f'Estimated Cost: GH₵ {total_str}\n'
        f'Pickup Date: {booking.pickup_date.strftime("%b %d, %Y")}\n\n'
        f'Track your shipment anytime at {track_url} using the tracking number above.\n\n'
        f'— Honey Cave Shipping (HCL)'
    )
    if booking.sender_email:
        try:
            send_mail(
                subject=f'Shipment Booked — Tracking #{booking.tracking_code}',
                message=email_body,
                from_email=django_settings.DEFAULT_FROM_EMAIL,
                recipient_list=[booking.sender_email],
                fail_silently=False,
            )
        except Exception as e:
            logger.warning('Shipment confirmation email failed for %s: %s', booking.tracking_code, e)

    if booking.sender_phone:
        wa_text = (
            f'📦 Honey Cave Shipping: Your shipment is booked!\n'
            f'Tracking #: {booking.tracking_code}\n'
            f'{booking.pickup_city} → {booking.dest_city}\n'
            f'Total: GH₵ {total_str}\n'
            f'Track it anytime at {track_url}'
        )
        sms_text = f'Honey Cave Shipping: Booked! Tracking #{booking.tracking_code}. Total GH₵{total_str}. Track: {track_url}'
        try:
            notify_phone(phone=booking.sender_phone, whatsapp_text=wa_text, sms_text=sms_text)
        except Exception as e:
            logger.warning('Shipment confirmation phone send failed for %s: %s', booking.tracking_code, e)

    # Logged-in customers also get an in-app notification tied to their account.
    if booking.booked_by_id:
        try:
            notify(
                ...
                message=f'Tracking #{booking.tracking_code} — {booking.pickup_city} → {booking.dest_city}. Total GH₵ {total_str}.',
                link='/shipping-import-export/',
            )
        except Exception as e:
            logger.warning('Shipment in-app notification failed for %s: %s', booking.tracking_code, e)
...
def shipping_track(request):
    """AJAX: look up a shipment by tracking code and return its status + timeline."""
    code = (request.GET.get('code') or '').strip().upper()
    if not code:
        return JsonResponse({'ok': False, 'error': 'Please enter a tracking number.'}, status=400)

    booking = ShipmentBooking.objects.filter(tracking_code=code).first()
    if not booking:
        return JsonResponse({'ok': False, 'error': f'No shipment found for "{code}". Please check the code and try again.'}, status=404)
    ...
    return JsonResponse({
        ...
        'origin': f'{booking.pickup_city}, {booking.pickup_country}',
        'destination': f'{booking.dest_city}, {booking.dest_country}',
        'status': booking.status,
        'status_display': booking.get_status_display(),
        'current_location': booking.current_location,
        'weight_kg': str(booking.weight_kg),
        'shipping_method': booking.get_shipping_method_display(),
        'delivery_option': SHIPPING_DELIVERY_OPTIONS.get(booking.delivery_option, {}).get('name', booking.delivery_option),
...
def my_shipments(request):
    """List the logged-in customer's own Shipping (HCL) bookings, most recent first."""
    shipments = (ShipmentBooking.objects
    ...
    return render(request, 'mall/my_shipments.html', {
        'shipments': shipments,
        'status_filter': status_filter,
        'status_choices': ShipmentBooking.STATUS_CHOICES,
    })


# ─── Own a Store — applicant flow ─────────────────────────────────────────────

@login_required
def store_application(request):
    existing = (StoreApplication.objects
                .filter(applicant=request.user)
                .order_by('-created')
                .first())

    # Already has a pending or approved application — show status, not the form.
    if existing and existing.status in ('pending', 'approved'):
        return render(request, 'mall/store_application.html', {
            'application': existing,
        })

    form = StoreApplicationForm(request.POST or None, request.FILES or None)
    if request.method == 'POST':
        if not check_rate_limit('store_application', request, limit=3, window=3600):
            messages.error(request, 'Too many submissions. Please try again later.')
            return redirect('store_application')
        if form.is_valid():
            ...
            notify_admins(
                ...
                message=(
                    f'{request.user.get_full_name() or request.user.username} applied to open '
                    f'"{application.store_name}" ({application.product_category}).'
                ),
                link='/panel/store-applications/',
            )
...
def ai_chat_message(request):
    """
    Server-side proxy for the AI chat widget. The browser must never call
    api.anthropic.com directly — that would require shipping the Anthropic
    API key to every visitor's browser, exposing it to anyone who opens
    devtools. This view holds the key server-side and forwards only the
    conversation.

    Available to guests as well as logged-in users (a customer shouldn't
    need an account just to ask "what are your delivery areas"). Order-
    specific context is only added for authenticated users via
    ai_chat_context, same as before.

    POST body (JSON): { "messages": [{"role": "user"/"assistant", "content": "..."}, ...],
    ...
    if not isinstance(raw_messages, list) or not raw_messages:
        return JsonResponse({'ok': False, 'error': 'No messages supplied.'}, status=400)

    # Only forward role/content pairs Anthropic accepts, capped so a caller
    # can't send an unbounded payload or inject unexpected fields.
    messages = []
    ...
    if not messages:
        return JsonResponse({'ok': False, 'error': 'No valid messages supplied.'}, status=400)

    system_prompt = str(body.get('system', ''))[:4000] or (
        'You are a helpful customer service assistant for Honey Cave Market, '
        'a Ghanaian e-commerce store. Be concise and friendly.'
    )
