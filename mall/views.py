...
def login_view(request):
    ...
    agree_terms_error = None
    if request.method == 'POST':
        if not request.POST.get('agree_terms'):
            agree_terms_error = 'You must agree to the Terms & Conditions and Privacy Policy to sign in.'
        elif form.is_valid():
            user = form.get_user()
            login(request, user)
    ...
    return render(request, 'mall/login.html', {'form': form, 'agree_terms_error': agree_terms_error})
